...
class GP:
    ...
    def init(self, X, Y, omega=None, training=None):
        ...
        self.wL = dot(
            inv(
                dot(Xt[training, :].T, Xt[training, :]) + eye(d) * var(Yt[training, :])
            ),
            dot(Xt[training, :].T, Yt[training, :]),
        )
        Yt = Yt - dot(X, self.wL)
        ...
        if method == "ANN":
            ...
            for i in range(len(layers) - 1):
                W = (2 * self.rng.random(layers[i], layers[i + 1]) - 1) / sqrt(
                    layers[i]
                )
            ...
            gamma = sqrt(2 * power(m, 1.0 / d) / mean(Dxy(X[training, :], P), 0))
            ...
            theta = concatenate(
                [P.flatten(), GAMMA.flatten(), lnAlpha.flatten(), b.flatten()]
            )
            f = self.GPz
        ...
        if heteroscedastic:
            ...
            w, SIGMAi = f(
                concatenate([theta.flatten(), zeros(2 * m * k).flatten()]),
                Xt,
                Yt,
                omega=omega,
                training=training,
            ...
            theta = concatenate(
                [theta.flatten(), zeros((m, k)).flatten(), lnEta.flatten()]
            )
    ...
    def train(
        ...
        result = minimize(
            ...
            method="L-BFGS-B",
            jac=True,
            args=(Xt, Yt, omega, training, validation),
    ...
    def ANN(
        self, theta, X, Y, omega=None, training=None, validation=None, returnModel=False
    ):
        ...
        for i in range(n_layers):
            W[i] = theta[ind[i] : ind[i] + layers[i] * layers[i + 1]].reshape(
                layers[i], layers[i + 1]
            )
            bias[i] = theta[
                ind[i]
                + layers[i] * layers[i + 1] : ind[i]
                + layers[i] * layers[i + 1]
                + layers[i + 1]
        ...
        if heteroscedastic:
            u = theta[
                ind[-1] + a_dim * k + k : ind[-1] + a_dim * k + k + m * k
            ].reshape(m, k)
        ...
        for i in range(k):
            ...
            variance[:, i : i + 1] = sum(
                dot(PHI[-1], SIGMAi[:, :, i]) * PHI[-1], 1
            ).reshape(n, 1)

            dwda[:, i : i + 1] = -dot(
                SIGMAi[:, :, i], alpha[:, i : i + 1] * w[:, i : i + 1]
            )

            dPHI = dPHI - dot(BxPHI, SIGMAi[:, 0:m, i]) * (1 - PHI[-1][:, 0:m] ** 2)
            dlnAlpha[:, i : i + 1] = (
                -0.5 * diag(SIGMAi[:, :, i]).reshape(a_dim, 1) * alpha[:, i : i + 1]
            )
        ...
        self.trainLL = mean(-0.5 * delta**2 / sigma - 0.5 * log(sigma)) - 0.5 * log(
            2 * pi
        )
        ...
        if validation is not None:
            ...
            self.validLL = mean(
                -0.5 * delta**2 / sigma - 0.5 * log(sigma)
            ) - 0.5 * log(2 * pi)
    ...
    def GPz(
        ...
        for i in range(k):
            ...
            variance[:, i : i + 1] = sum(dot(PHI, SIGMAi[:, :, i]) * PHI, 1).reshape(
                n, 1
            )
        ...
        if heteroscedastic:
            u = theta[
                m * d + g_dim + a_dim * k + k : m * d + g_dim + a_dim * k + k + m * k
            ].reshape(m, k)
            lnEta = theta[
                m * d
                + g_dim
        ...
        for j in range(m):
            ...
            if method == "GL":
                ...
                dGAMMA = dGAMMA - diag(
                    dot(
                        GAMMA.reshape(d, 1) * (Delta * dlnPHI[:, j].reshape(n, 1)).T,
                        Delta,
                    )
                )
            elif method == "VD":
                ...
                dGAMMA = dGAMMA - dot(
                    dot(GAMMA, (Delta * dlnPHI[:, j].reshape(n, 1)).T), Delta
                )
            else:
                dP[j, :] = dot(
                    dot(dlnPHI[:, j].T, Delta), dot(GAMMA[:, :, j].T, GAMMA[:, :, j])
                )
                dGAMMA[:, :, j] = -dot(
                    dot(GAMMA[:, :, j], (Delta * dlnPHI[:, j].reshape(n, 1)).T), Delta
                )
        ...
        if heteroscedastic:
            ...
            grad = concatenate(
                (dP.flatten(), dGAMMA.flatten(), dlnAlpha.flatten(), db.flatten())
            )
    ...
    def getPHI(self, theta, X):
        ...
        b = theta[m * d + g_dim + a_dim * k : m * d + g_dim + a_dim * k + k].reshape(
            1, k
        )
    ...
    def callbackF(self, theta):
        if self.iter == 1:
            if self.validLL is None:
                print(
                    "{0:4s}\t{1:9s}\t\t{2:9s}\t\t{3:9s}\t\t{4:9s}".format(
                        "Iter", " logML/n", " Train RMSE", " Train MLL", " Time"
                    )
                )
            else:
                print(
                    "{0:4s}\t{1:9s}\t\t{2:9s}\t\t{3:9s}\t\t{4:9s}\t\t{5:9s}\t\t{6:9s}".format(
                        "Iter",
                        " logML/n",
                        " Train RMSE",
                        " Train RMSE/n",
        ...
        if self.validLL is None:
            print(
                "{0:4d}\t{1: 1.7e}\t{2: 1.7e}\t{3: 1.7e}\t{4: 1.7e}".format(
                    self.iter,
                    -self.nlogML,
                    self.trainRMSE,
                    self.trainLL,
            ...
            if self.iter == 1 or self.validLL >= self.best_valid:
                print(
                    "{0:4d}\t{1: 1.7e}\t{2: 1.7e}\t{3: 1.7e}\t{4: 1.7e}\t[{5: 1.7e}]\t{6: 1.7e}".format(
                        self.iter,
                        -self.nlogML,
                        self.trainRMSE,
                        self.trainLL,
