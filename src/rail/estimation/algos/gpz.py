...
def _prepare_data(
    data_dict, bands, err_bands, nondet_val, maglims, logflag, repl_err_vals
):
    ...
    for i, (band, eband, lim, rplval) in enumerate(
        zip(bands, err_bands, maglims.values(), repl_err_vals)
    ):
        data[:, i] = data_dict[band]
        mask = np.bitwise_or(
            np.isclose(data_dict[band], nondet_val), ~np.isfinite(data_dict[band])
        )
        data[:, i][mask] = lim
        errband = data_dict[eband]
        emask = np.bitwise_or(errband <= 0.0, ~np.isfinite(errband))
...
class GPzInformer(CatInformer):
    ...
    config_options.update(
        ...
        gpz_method=Param(
            str,
            "VC",
            msg="method to be used in GPz, options are 'GL', 'VL', 'GD', 'VD', 'GC', and 'VC'",
        ),
        n_basis=Param(int, 50, msg="number of basis functions used"),
        learn_jointly=Param(
            bool, True, msg="if True, jointly learns prior linear mean function"
        ),
        hetero_noise=Param(
            bool,
            True,
            msg="if True, learns heteroscedastic noise process, set False for point est.",
        ),
        csl_method=Param(
            str,
            "normal",
            msg="cost sensitive learning type, 'balanced', 'normalized', or 'normal'",
        ),
        csl_binwidth=Param(
            float, 0.1, msg="width of bin for 'balanced' cost sensitive learning"
        ),
        pca_decorrelate=Param(
            bool, True, msg="if True, decorrelate data using PCA as preprocessing stage"
        ),
    ...
    def run(self):
        ...
        if not np.logical_and(
            len(self.config.bands) == len(self.config.err_bands),
            len(self.config.err_bands) == len(self.config.replace_error_vals),
        ):  # pragma: no cover
            raise ValueError(
        ...
        input_array = _prepare_data(
            ...
            self.config.replace_error_vals,
        )
